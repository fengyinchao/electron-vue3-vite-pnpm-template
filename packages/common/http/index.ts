/*
 * @Description: http 封装，支持拦截器
 * @Author: Feng Yinchao
 * @Date: 2024-08-09 09:09:02
 */
import { isMain } from '@common/const'
...
function addRequestInterceptor(onFulfilled: RequestInterceptor, onRejected?: RequestInterceptor): number {
  return requestInterceptors.use(onFulfilled)
}

function addResponseInterceptor(onFulfilled: ResponseInterceptor<any>, onRejected?: ResponseInterceptor<any>): number {
...
const electronRequest = (electronAdaptedOptions: Options) => {
  ...
  const netPromise = new Promise((resolve, reject) => {
    ...
    if (electronAdaptedOptions.headers && Object.keys(electronAdaptedOptions.headers).length) {
      Object.keys(electronAdaptedOptions.headers).forEach((key: string) => {
